...
export default function SubmissionResultPage() {
  ...
  if (loading) return (
    <><Navbar /><div style={{ textAlign: "center", padding: "80px", color: "#1e7ab8" }}><i className="fas fa-spinner fa-spin" style={{ fontSize: "2rem" }} /></div></>
  );
  ...
  const scoreColor = pct >= 80 ? "#2e7d32" : pct >= 50 ? "#e65100" : "#1e7ab8";
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "28px 20px" }}>
        {/* Result summary */}
        <div style={{ background: "#fff", borderRadius: "20px", border: "2px solid #c5ddf0", padding: "36px", textAlign: "center", marginBottom: "24px" }}>
        ...
        {/* Detailed review */}
        <div style={{ background: "#fff", borderRadius: "16px", border: "2px solid #c5ddf0", padding: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
            <h2 style={{ fontFamily: "Nunito, sans-serif", fontWeight: 800, fontSize: "1rem", color: "#2c2c2c" }}>
              Chi tiết đáp án
            </h2>
            <button onClick={() => setShowAll((s) => !s)} style={{ background: "none", border: "1.5px solid #c5ddf0", borderRadius: "8px", padding: "6px 14px", fontSize: "0.8rem", cursor: "pointer", color: "#555" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            {(showAll ? result.questions : result.questions.slice(0, 5)).map((q, i) => (
              <div key={q.id} style={{
                border: `2px solid ${q.correct ? "#4caf5044" : "#2196f344"}`,
                borderRadius: "12px", padding: "16px",
                background: q.correct ? "#e8f5e944" : "#e3f2fd44",
              }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "8px" }}>
                  <span style={{ background: q.correct ? "#4caf50" : "#2196f3", color: "#fff", borderRadius: "50%", width: "22px", height: "22px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem" }}>
                  ...
                  {(["A", "B", "C", "D"] as const).map((opt) => {
                    ...
                    return (
                      <span key={opt} style={{
                        ...
                        background: isCorrect ? "#e8f5e9" : isUser ? "#e3f2fd" : "#f5f5f5",
                        color: isCorrect ? "#2e7d32" : isUser ? "#1970a8" : "#777",
                        border: isCorrect ? "1.5px solid #4caf50" : isUser ? "1.5px solid #2196f3" : "1.5px solid #eee",
                      }}>
                        {opt}. {q[key]}
          ...
          {!showAll && result.questions.length > 5 && (
            <button onClick={() => setShowAll(true)} style={{ width: "100%", marginTop: "12px", background: "#f0f7fd", border: "2px solid #c5ddf0", borderRadius: "10px", padding: "10px", cursor: "pointer", color: "#1e7ab8", fontWeight: 600, fontSize: "0.85rem" }}>
        ...
        <div style={{ marginTop: "20px", display: "flex", gap: "12px" }}>
          <Link href="/practice/history" style={{ flex: 1, textAlign: "center", background: "#fff", border: "2px solid #c5ddf0", borderRadius: "12px", padding: "12px", color: "#555", fontWeight: 600, fontSize: "0.875rem" }}>
            <i className="fas fa-history" style={{ marginRight: "8px" }} />Lịch sử
          </Link>
          <Link href="/practice" style={{ flex: 1, textAlign: "center", background: "#1e7ab8", color: "#fff", borderRadius: "12px", padding: "12px", fontWeight: 600, fontSize: "0.875rem" }}>
